...
//         if (rx_buff[i] == '\n')
//         {
//             std::string currentString(reinterpret_cast<char*>(rx_buff + start), i - start);
//             incoming_commands_q.push(currentString);
//             start = i + 1;
//             int nothing = 0;
//             if (rx_buff[i + 1] != 'c')
...
// #ifdef ECHO_INCOMMING
//     const std::string& back_command = incoming_commands_q.back();
//     CHAL_UART2_SendString(back_command.c_str());
// #endif
...
// void CHAL_push_to_q(uint8_t* rx_buff, uint16_t bufferlength)
// {
//     static size_t read_position = 0;
//     size_t write_position = (bufferlength - DMA1_Stream5->NDTR) % bufferlength; // get the current write position from the DMA

//     while (read_position != write_position)
//     {
//         size_t current_line_start = read_position;
//         bool line_found = false;

//         // Search for the next newline character
//         while (read_position != write_position)
//         {
//             if (rx_buff[read_position] == '\n')
//             {
//                 line_found = true;
//                 break;
//             }
//             read_position = (read_position + 1) % bufferlength;
//         }

//         if (line_found)
//         {
//             // Copy the string up to and including the newline character
//             size_t line_length = (read_position >= current_line_start) ? (read_position - current_line_start) : (bufferlength - current_line_start + read_position);

//             std::string currentString((char*)rx_buff + current_line_start, line_length);
//             incoming_commands_q.push(currentString);
...
//         else
//         {
//             // No more complete lines in the buffer
//             break;
//         }
//     }

// #ifdef ECHO_INCOMMING
//     if (!incoming_commands_q.empty())
//     {
//         const std::string& back_command = incoming_commands_q.back();
//         CHAL_UART2_SendString((char*)back_command.c_str());
//     }
// #endif
//     CHAL_disable_DMA(DMA1_Stream5); // to change the NDTR register the DMA NEEDS to be disabled first.
//     DMA1_Stream5->NDTR = bufferlength; // reset RX-buff pointer to start
...
//     // Reset the read position for the next buffer processing
//     read_position = 0;
// }
